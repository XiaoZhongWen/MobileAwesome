...
class DataUtils {
  ...
  static Future<String> getThemeColor() async {
    ...
    return response['success'] as Future<String>;
  }
  ...
  static Future<List> getWidgetTreeList() async {
    try {
      ...
      if (response != null && response['success'] as bool) {
        return response['data'] as Future<List>;
      } else {
        return [];
      }
    } catch (error) {
  ...
  static Future getAllCollections(BuildContext context) async {
    ...
    if (response != null && response['success'] == true) {
      for (int i = 0; i < (response['data'].length as num); i++) {
        Map<String, dynamic> tempCo = response['data'][i] as Map<String, dynamic>;
  ...
  static Future searchWidget(String name) async {
    ...
    if (response != null && response['success'] == true) {
      for (int i = 0; i < (response['data'].length as num); i++) {
        ...
        String routerName;
        if (json['display'] == 'old') {
          routerName = json['path'] as String;
        } else {
          routerName = json['pageId'] as String;
        }
        Map<String, dynamic> tempMap = {
          ...
          "catId": json['parentId'].runtimeType == String
              ? int.parse(json['parentId'] as String)
              : json['parentId']
        };
